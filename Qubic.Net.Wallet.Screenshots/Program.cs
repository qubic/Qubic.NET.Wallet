...
class Program
{
    ...
    static Episode[] BuildEpisodes() => new Episode[]
    {
        ...
        new(9, "MSVault Deep Dive",      "09_msvault",            true,  true,  new Func<IPage, Task>[]
            { Chapter12_MSVault }),
    };
    ...
    static void PrintUsage()
    {
        Console.WriteLine("""
            ...
            Episodes:
              ...
              9  MSVault Deep Dive     — Full MSVault walkthrough (standalone)
    ...
    {
        const string ch = "12_msvault";
        await Nav(page, "msvault");

        // [SCENE: overview] — Page header + "Your Vaults" panel (or empty state)
        await ScreenshotAndHold(page, ch, "overview");

        // [SCENE: vault_summary] — Vault cards with balance, owners, action buttons
        var vaultCard = page.Locator("div.card.border-primary").First;
        if (await vaultCard.IsVisibleAsync())
        {
            await ScreenshotAndHold(page, ch, "vault_summary");

            // [SCENE: pending_release] — Pending release card with approval status
            var pendingRelease = page.Locator("div.card.border-warning, div.card.border-info").First;
            if (await pendingRelease.IsVisibleAsync())
            {
                await pendingRelease.ScrollIntoViewIfNeededAsync();
                await Pace();
                await ScreenshotAndHold(page, ch, "pending_release");
            }
        }

        // [SCENE: tab_register] — Register tab with empty form
        await ClickTab(page, "Register");
        await ScreenshotAndHold(page, ch, "tab_register");

        // [SCENE: register_filled] — Fill in some demo data
        var vaultNameInput = page.Locator("input[placeholder*='My Vault']").First;
        if (await vaultNameInput.IsVisibleAsync())
        {
            await vaultNameInput.FillAsync("Team Treasury");
            await Task.Delay(500);
        }
        // Fill first owner with own identity if available
        var selfIdentity = await page.EvaluateAsync<string?>(
            "() => document.querySelector('.qb-seed-bar code')?.textContent?.trim()");
        var ownerInputs = page.Locator("input.mono[placeholder*='IDENTITY']");
        if (await ownerInputs.CountAsync() >= 2)
        {
            if (selfIdentity != null)
                await ownerInputs.First.FillAsync(selfIdentity);
            await Task.Delay(300);
            await ownerInputs.Nth(1).FillAsync("BAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA");
            await Task.Delay(300);
        }
        // Add a third owner
        var addOwnerBtn = page.Locator("button:has-text('+ Add Owner')").First;
        if (await addOwnerBtn.IsVisibleAsync())
        {
            await addOwnerBtn.ClickAsync();
            await Task.Delay(500);
            var ownerCount = await ownerInputs.CountAsync();
            if (ownerCount >= 3)
                await ownerInputs.Nth(ownerCount - 1).FillAsync("CAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA");
            await Task.Delay(300);
        }
        var approvalsInput = page.Locator("input[type='number'][min='2']").First;
        if (await approvalsInput.IsVisibleAsync())
        {
            await approvalsInput.FillAsync("2");
            await Task.Delay(300);
        }
        await ScreenshotAndHold(page, ch, "register_filled");

        // [SCENE: tab_deposit] — Deposit tab
        await ClickTab(page, "Deposit");
        var depAmountInput = page.Locator("input[type='number'][min='1']").First;
        if (await depAmountInput.IsVisibleAsync())
        {
            await depAmountInput.FillAsync("10000");
            await Task.Delay(300);
        }
        await ScreenshotAndHold(page, ch, "tab_deposit");

        // [SCENE: tab_release] — Release tab in release mode
        await ClickTab(page, "Release");
        await ScreenshotAndHold(page, ch, "tab_release");

        // [SCENE: release_reset] — Switch to Reset mode
        var resetRadio = page.Locator("#relModeReset");
        if (await resetRadio.IsVisibleAsync())
        {
            await resetRadio.ClickAsync();
            await Pace();
            await ScreenshotAndHold(page, ch, "release_reset");
        }

        // [SCENE: tab_assets] — Assets tab
        await ClickTab(page, "Assets");
        await ScreenshotAndHold(page, ch, "tab_assets");

        // [SCENE: tab_vote_fee] — Vote Fee tab (only visible when vault exists)
        var voteFeeTab = page.Locator("ul.nav-tabs a.nav-link:has-text('Vote Fee')");
        if (await voteFeeTab.IsVisibleAsync())
        {
            await voteFeeTab.ClickAsync();
            await Pace();
            await ScreenshotAndHold(page, ch, "tab_vote_fee");
        }

        // [SCENE: tab_query] — Query tab with function selector
        await ClickTab(page, "Query");
        await ScreenshotAndHold(page, ch, "tab_query");

        // [SCENE: query_result] — Run a query to show results
        var queryBtn = page.Locator("button:has-text('Query')").First;
        if (await queryBtn.IsVisibleAsync() && await queryBtn.IsEnabledAsync())
        {
            await queryBtn.ClickAsync();
            await Task.Delay(3000);
            await WaitForBlazor(page);
            var resultTable = page.Locator("table.table-bordered").First;
            if (await resultTable.IsVisibleAsync())
            {
                await ScreenshotAndHold(page, ch, "query_result");
            }
        }

        // [SCENE: tab_history / history_table] — History tab (added dynamically)
        var historyTab = page.Locator("ul.nav-tabs a.nav-link:has-text('History')");
        if (await historyTab.IsVisibleAsync())
        {
            await historyTab.ClickAsync();
            await Pace();
            await WaitForBlazor(page);
            await ScreenshotAndHold(page, ch, "tab_history");

            // Wait for history to load
            await Task.Delay(2000);
            var histTable = page.Locator("table.table-hover").First;
            if (await histTable.IsVisibleAsync())
            {
                await ScreenshotAndHold(page, ch, "history_table");
            }
        }

        // [SCENE: outro] — Return to overview for closing shot
        await page.EvaluateAsync("() => window.scrollTo(0, 0)");
        await Pace();
        await ScreenshotAndHold(page, ch, "outro");
